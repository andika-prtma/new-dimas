...
class Welcome extends CI_Controller {
	...
	public function cekUser(){
		$post = [
			'username' 	=> 'rizky.musthofa@multifab.co.id',
			'password' 	=> 'admin',
			'auth' 		=> $this->db->auth,
		];
	...
	public function updateUser(){
		...
		$login = getAllUser()->data;

		//var_dump($login);
		foreach ($login as $x) {
			echo $x->badge_number.' = '.$x->full_name.' = '.$x->email.'<br>';
			
			// if ($x->email != 'rizky.musthofa@multifab.co.id') {			
			// 	$data = [
			// 		'id_dept' 		=> 1,
			// 		'id_role'		=> 2,
			// 		'badge_number' 	=> $x->badge_number,
			// 		'email'			=> $x->email,
			// 		'first_name' 	=> $x->full_name,
			// 		'created_at'	=> time()
			// 	];
